...
public class ClickHouseAlterTableTest extends com.altinity.clickhouse.sink.connector.db.operations.ClickHouseAutoCreateTableTest {
    ...
    public void createAlterTableSyntaxTest() {
        ...
        // Add Column
        String alterTableAddColumnQuery = cat.createAlterTableSyntax("employees", null,
        ...
        // Delete Column
        String alterTableDeleteColumnQuery = cat.createAlterTableSyntax("employees", null,
        ...
        // Add Column on cluster
        String alterTableClusterAddColumnQuery = cat.createAlterTableSyntax("employees", "{cluster}",
                this.getExpectedColumnToDataTypesMap(), ClickHouseAlterTable.ALTER_TABLE_OPERATION.ADD);

        String expectedAddColumnClusterQuery = "ALTER TABLE employees ON CLUSTER '{cluster}' add column `amount` Float64,add column `occupation` String,add column `quantity` Int32,add column `blob_storage_scale` Decimal,add column `json_output` JSON,add column `max_amount` Float64,add column `amount_1` Float32,add column `customerName` String,add column `blob_storage` String,add column `employed` Bool";
        Assert.assertTrue(alterTableClusterAddColumnQuery.equalsIgnoreCase(expectedAddColumnClusterQuery));
    }
}
